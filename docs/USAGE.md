...
```
課題#9178のステータスを完了に変更し、進捗を100%に更新してください
（パラメータ: issue_id = 9178, status_id = 5, done_ratio = 100, notes = "作業完了"）
```

```
課題#456の担当者を変更し、期日を来月末に設定してください
（パラメータ: issue_id = 456, assigned_to_id = 12, due_date = "2025-07-31"）
```

### 一括課題更新 🆕 新機能

```
課題#9178と#9180を一括でクローズしてください
（パラメータ: issue_ids = [9178, 9180], status_id = 5, notes = "一括完了処理"）
```

```
複数の課題を特定のユーザーに一括で割り当ててください
（パラメータ: issue_ids = [100, 101, 102], assigned_to_id = 15）
```
